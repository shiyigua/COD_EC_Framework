...
/**
 * @brief structrue that contains the information of Gimbal motors
*/
...
void DJI_Motor_Info_Update(uint32_t *StdId, uint8_t *rxBuf,DJI_Motor_Info_Typedef *DJI_Motor)
{
	...
	switch(DJI_Motor->Type)
	{
		...
		case DJI_M3508:
			...
			if(*StdId == DJI_Motor[TRIGGER].Data.StdId)
			{
				DJI_Motor->Data.angle = encoder_to_anglesum(&DJI_Motor->Data,51.f,8192);
			}
		break;
