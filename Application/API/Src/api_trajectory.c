...
void SolveTrajectory_Transform(MiniPC_SendPacket_Typedef *MiniPCTxData,MiniPC_ReceivePacket_Typedef *MiniPCRxData,SolveTrajectory_Typedef *SolveTrajectory)
{
    ...
    SolveTrajectory->yaw_calc += SolveTrajectory->yawgyro_calc * timeDelay;
		...
		else if(SolveTrajectory->yawgyro_calc < 0)
		{
			sign_yawgyro = 1;
		}
    ...
    if (SolveTrajectory->armors_num == 2) 
    {
        ...
        float yaw_diff_min = fabsf(SolveTrajectory->armorlock_yaw - SolveTrajectory->target_posure[0].yaw);
        float temp_yaw_diff = fabsf(SolveTrajectory->armorlock_yaw - SolveTrajectory->target_posure[1].yaw);
    ...
    else if (SolveTrajectory->armors_num == 3)
    {
        ...
        for (uint8_t i = 0; i<3; i++)
        {
            ...
            SolveTrajectory->target_posure[i].yaw = SolveTrajectory->yaw_calc + i * 2.f*PI/3.f;
						...
						{
							SolveTrajectory->target_posure[i].yaw -= SolveTrajectory->target_posure[i].yaw/fabsf(SolveTrajectory->target_posure[i].yaw)*2*PI;
						}
        }
        /* select the minimum yaw armor */
        float yaw_diff_min = fabsf(SolveTrajectory->centerlock_yaw - SolveTrajectory->target_posure[0].yaw);
        for (uint8_t i = 1; i<3; i++) 
        {
            float temp_yaw_diff = fabsf(SolveTrajectory->centerlock_yaw - SolveTrajectory->target_posure[i].yaw);
            if (temp_yaw_diff < yaw_diff_min)
            {
                yaw_diff_min = temp_yaw_diff;
                index = i;
...
//				myprintf((int16_t)(SolveTrajectory->target_posure[0].yaw*57.295779513f*100),(int16_t)(SolveTrajectory->target_posure[index].yaw*57.295779513f*100));
    }
...
//            SolveTrajectory->target_posure[i].x = MiniPCRxData->x - r*cos(SolveTrajectory->yaw_calc + sign_yawgyro  * i * PI/2.f);
//            SolveTrajectory->target_posure[i].y = MiniPCRxData->y - r*sin(SolveTrajectory->yaw_calc + sign_yawgyro  * i * PI/2.f);
//            SolveTrajectory->target_posure[i].z = use_1 ? MiniPCRxData->z : (SolveTrajectory->dz +MiniPCRxData->z);
...
//            use_1 = !use_1;
					...
					SolveTrajectory->target_posure[i].yaw = SolveTrajectory->yaw_calc + sign_yawgyro * i * PI/2.f;
					if(fabsf(SolveTrajectory->target_posure[i].yaw) > 2*PI)
					{
